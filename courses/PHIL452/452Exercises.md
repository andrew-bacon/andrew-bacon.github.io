...
In this exercise you will show that the formula 

	$\mathsf{L}$: $\Box(\Box P\to P) \to \Box P$

characterizes the class of frames which are transitive and are converse well-founded (there are no infinite sequences $x_1, x_2, x_3, ...$ where $Rx_ix_{i+1}$ for all $i$.
...
2. Here you will show that the property of being irreflexive is not frame definable. Consider the irreflexive frame $\mathcal{F} = (\{x,y\}, \{(x,y),(y,x)\}$ and the reflexive frame $\mathcal{G}=(\{w\}, \{(w,w)\}$. Given a valuation $V$ on $\mathcal{G}$ define a valuation $V'$ on $\mathcal{F}$ by $V'(P)=\{x,y\}$ if $V(P)=\{w\}$ and $V'(P)=\emptyset$ if $V(P)=\emptyset$.
